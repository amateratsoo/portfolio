...
import { ReactNode, SVGAttributes, CSSProperties } from 'react'
...
type ParticleProps = SVGAttributes<SVGElement>
...
const SparkleButton = (props: SparkleButtonProps) => {
  ...
  return (
    <div
      className={cn(
        'relative inline-block [--spark:1.8s] [--transition:0.25s] rounded-full p-[0.10rem] bg-gradient-to-r from-accent-color/25 to-[#666666]',
        className
      )}
    >
      {children}
      <span
        aria-hidden='true'
        className='absolute left-1/2 top-1/2 -z-10 aspect-square w-[200%] -translate-x-1/2 -translate-y-1/2 opacity-[var(--active,0)] [-webkit-mask:radial-gradient(--accent-color,transparent_65%)] [transition:opacity_var(--transition)] peer-hover:[--active:1] peer-hover:[--play-state:running]'
      >
...
const Text = (props: TextProps) => {
  ...
  return <span className='translate-x-[2%] translate-y-[-6%]'>{children}</span>
}
